...
function displayMessage(msg) {
    ...
    document.querySelector('.contact-list')
        .querySelector(
            `[data-id="${message[0] !== userId ? message[0] : message[1]}"]`
        )
        .querySelector(".msg").textContent = message[2];
}
...
function addContact(messageParts) {
    ...
    contacts.appendChild(card);
    console.log(
        contacts.querySelector(`.contact-item[data-id="${messageParts[1]}"]`)
    );
    contacts
        .querySelector(`.contact-item[data-id="${messageParts[1]}"]`)
        .addEventListener("click", () => {
            contacts.appendChild(card);
            document.querySelector("#messages").style.display = "block";
            document.querySelector(".message-input-container").style.display =
                "flex";
            receiverId = messageParts[1];
            highlightSelectedContact(
                document.querySelector(
                    `.contact-item[data-id="${messageParts[1]}"]`
                )
            );
        });
...
function addFriendRequest(req) {
    ...
    abtn.addEventListener("click", function() {
        ...
        noti.removeChild(this.parentElement);
    });
...
function addContactToList(senderId, name) {
    ...
    card.querySelector(".msg").textContent = "No messages";
    contacts.appendChild(card);
    console.log(contacts.querySelector(`.contact-item[data-id="${senderId}"]`));
    contacts
        .querySelector(`.contact-item[data-id="${senderId}"]`)
        .addEventListener("click", () => {
            document.querySelector("#messages").style.display = "block";
            document.querySelector(".message-input-container").style.display =
                "flex";
            receiverId = senderId;
            highlightSelectedContact(
                document.querySelector(`.contact-item[data-id="${senderId}"]`)
            );
        });
